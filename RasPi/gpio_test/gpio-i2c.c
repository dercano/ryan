...
// Set Stop (when set to stop, oscillator of DS1307 is disabled)
void setStop (int fd)
{
  unsigned char temp;
...
// Set Start (when set to start, oscillator of DS1307 is enabled)
...
void setClock (int fd)
{
  ...
  clock [3] = D2B(t.tm_wday+1) & 0x07; // weekdays (sun 0) -> ds1307 weekdays (sun 1)
